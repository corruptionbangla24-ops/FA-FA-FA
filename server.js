...
// 🀄 ৩-রিলস ওরিজিনাল ফা ফা ফা ট্র্যাডিশনাল ক্যাসিনো প্রতীক তালিকা ভাই ভাই
const fafafaPool = ["GOD_OF_WEALTH", "GOLD_SYCEE", "RED_ENVELOPE", "LUCKY_8", "CHINESE_COIN", "ORANGE_SIGN", "FORTUNE_GONG"];

// 💰 ১. লাইভ অ্যাকাউন্ট ব্যালেন্স নিয়ে আসার ডেডিকেটেড গেটওয়ে
app.get('/api/fafafa-balance', async (req, res) => {
...
// 🛫 ২. ফা ফা ফা ৩-রিল কোর স্লট স্পিন রাউট (POST Route - ৯৫% RTP গাণিতিক বর্ম কঠোর লক ভাই ভাই!)
app.post('/api/fafafa-spin', async (req, res) => {
    ...
    try {
        ...
        let adminTriggeredPrize = (balResponse.data && balResponse.data.fafafa_target) ? balResponse.data.fafafa_target : null;

        let spinResults, finalStatus, winMultiplier;
        let isLoopActive = true;
        let loopSafety = 0;

        // 🎰 [🎰 ৯৫% ওরিজিনাল ক্যাসিনো RTP ও ৩-রিলস ফা ফা ফা গাণিতিক লুপ ভাই ভাই]
        while (isLoopActive && loopSafety < 200) {
            ...
            spinResults = [];
            for (let r = 0; r < 3; r++) {
                spinResults.push(fafafaPool[Math.floor(Math.random() * fafafaPool.length)]);
            }
            ...
            if (maxSameSymbolsCount === 3) {
                finalStatus = "win";
                // 🚀 [ডাইনামিক জ্যাকপট ওডস]: ৩টি হুবহু সেম মিললে ৫০০ গুণের মেগা গ্র্যান্ড জ্যাকপট!
                winMultiplier = 500.00;
            } else if (maxSameSymbolsCount === 2) {
                finalStatus = "win";
                // ২টি মিললে ১০ গুণ আংশিক রিটার্ন প্রফিট
                winMultiplier = 10.00;
            } else {
                finalStatus = "lose";
                winMultiplier = 0.00;
            ...
            if (adminTriggeredPrize) {
                ...
                // ৫০০ গুণের মেগা জ্যাকপটের চান্স আরটিপি লুপ ট্র্যাকে কড়া সুরক্ষায় টাইট ০.১% এ লক ভাই ভাই
                if (winMultiplier === 500.00 && Math.random() > 0.001) continue;
        ...
        console.error("Fa Fa Fa Core Engine Error:", e.message);
...
io.on('connection', (socket) => { console.log("Player connected to Royal Fa Fa Fa 3-Reel Slot Engine!"); });

// ফা ফা ফা গেম নিজস্ব কাস্টম ৬১০০ পোর্টে কড়া নিয়নে অন ফায়ার ভাই ভাই!
const PORT = process.env.PORT || 6100; 
server.listen(PORT, () => { console.log(`🎡 Royal Fa Fa Fa Engine Running on port ${PORT}`); });
